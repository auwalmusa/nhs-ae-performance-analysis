...
Two-page interactive Power BI report (`dashboard/nhs_ae_winter_performance.pbix`), designed with a custom theme and background canvas matching my portfolio brand.
...
The five business questions are answered as documented queries in [`sql/analysis_questions.sql`](sql/analysis_questions.sql) (CTEs, window functions, conditional-aggregation pivots), executed via DuckDB by `run_analysis.py`. Full results: [`reports/sql_answers.md`](reports/sql_answers.md).
...
|-- sql/                     business questions answered in SQL (DuckDB, ANSI)
|-- dashboard/               Power BI report, theme and design assets
|-- reports/                 SQL answers; written findings report (in progress)
```
